...
<template>
  <div class="rounded-2xl border border-white/5 bg-slate-900/50 p-4">
    <div class="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
      ...
      <template v-else>
        <div class="flex items-center gap-2">
          <select
            ...
            class="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-200 transition focus-visible:outline-none disabled:opacity-40"
          >
      ...
      <span
        v-if="isComplete"
        class="rounded-full bg-amber-400/15 px-3 py-1 text-xs font-medium text-amber-300 sm:ml-auto"
      >
        Season complete
      </span>
    </div>
